...
mapLoop:
    add t4, s0, x0      # load the address of the array of current node into temporary reg t5
    #add t5,t5,x0        # initializing a temp reg t5 to 0
    lw t2, 4(s0)        # load the size of the node's array into t2
    #add t1, t1, t0      # offset the array address by the count
    lw a0, 0(t4)        # load the value at that address into a0
    add a0,a0,t5        #add a0 + t5 (t5 is incremented by 4 every time so when in array it picks up every word after 4 bytes)
    jalr s1             # call the function on that value.

    sw t3, 0(a0)        # store the returned value back into the array
    addi t5,t5,4        #increment t5 by 4 so it picks next element of array
